...
can select as many as you like, and Kubus talks to all of them at once.
...
- **Actions stay scoped.** Deleting, scaling or restarting always targets the specific
  cluster the resource belongs to, so there's no accidental fan-out.
...
<figure markdown="span">
  ...
  <figcaption>One list, many clusters. The Cluster column tells them apart.</figcaption>
</figure>
...
only some of them is drawn outlined, and its tooltip names the clusters. Each cluster's
rows are still filtered by that cluster's own namespaces, so prod never shows `team-a`
just because dev is open next to it. Editing the filter applies to every selected cluster.
...
- **Add cluster** takes a pasted kubeconfig snippet or a short form (name, API server, CA,
  and either a bearer token or a client certificate). For either path you can choose
  direct access, a Kubus-managed SSH jump host, or an existing SOCKS/HTTP proxy. The new
  entry is merged into your kubeconfig, and a backup is written first.
- **Edit** (:material-pencil: on any row) changes a cluster's API server, credentials, TLS
  settings, and the connection options below (SSH jump host / proxy). Cloud-provider
  clusters that authenticate with an exec plugin (EKS/GKE/AKS) keep their existing login:
  leave **Credentials** on *Keep current* and only the other fields change.
- **Remove** (:material-delete-outline: on any row) deletes the context from your
...
Kubus treats managed clusters like any other, with one thing to understand: the kubeconfig
that `gcloud container clusters get-credentials`, `aws eks update-kubeconfig` or
`az aks get-credentials` writes contains **no credentials at all**. It only names a
*credential plugin* (`gke-gcloud-auth-plugin`, `aws`, `kubelogin`), which `kubectl` and
Kubus alike run to mint a short-lived token for every connection.
...
If a cluster's API server is only reachable from your machine through a bastion, VPN, or
SSH jump host, open its **Edit** dialog and pick a **Connection** under *"Only if this
cluster isn't reachable directly"*:
...
chains, per-host options. It works the same on macOS, Linux, and Windows (Windows needs
the built-in *OpenSSH Client* optional feature).
...
!!! tip "Already running your own `ssh -D`?"
    ...
    `socks5://localhost:1080` instead. The managed **SSH jump host** mode does the same
    for you, and also restarts the tunnel when it drops.
...
-   :material-view-dashboard: **[Overview dashboard](overview.md)** shows health across every selected cluster.
-   :material-cog: **[Settings → Clusters](settings.md)** is where you manage kubeconfig entries and protection.
