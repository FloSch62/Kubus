...
Kubus can show **live traffic between your pods**: who talks to whom, and at what rate.
It needs no Prometheus, service mesh, or specific CNI.
...
<figure markdown="span">
  ...
  <figcaption>Live pod-to-pod traffic without Prometheus or a service mesh.</figcaption>
</figure>
...
Links are **direction-neutral**. The agent observes packets and can't tell which side
opened the connection, so each link shows A→B and B→A rates. Rates are computed between
agent scrapes (~20 s apart), and history accumulates while Kubus is open.
...
- The agent pods use the **host network and elevated capabilities** (with a
  privileged init container), which eBPF needs. Linux nodes only.
