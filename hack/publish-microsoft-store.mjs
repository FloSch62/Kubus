...
const productId = '9PCTHB079SK7';
const identityName = 'FloSch.me.Kubus';
...
export async function publishStore(directory, version, run = runStore) {
  ...
  const expectedFile = `kubus-${version}-win-x64.appx`;
  ...
  if (!app.LastPublishedApplicationSubmission?.Id) {
    return 'Store upload skipped: publish the first release manually in Partner Center using the kubus-windows-store artifact. Later releases will submit automatically.';
  }
  if (app.PendingApplicationSubmission?.Id) {
    throw new Error('Kubus has a pending Store submission. Finish it in Partner Center, then rerun this job. The existing submission was left intact.');
  }
  ...
  return `Submitted Kubus ${version} (Store package ${target.join('.')}) for certification. Publication follows the Store submission's publishing schedule.`;
}
