...
switch (command) {
  case 'csr': {
    ...
    save('Kubus.certSigningRequest', openssl('req', '-new', '-sha256', '-key', file('developer-id.key.pem'),
      '-passin', `file:${file('private-key.password')}`, '-subj', '/CN=Florian Schwarz/emailAddress=schwarz.flori.88@googlemail.com'));
    openssl('req', '-in', file('Kubus.certSigningRequest'), '-verify', '-noout');
    console.log(`Upload this CSR to Apple: ${file('Kubus.certSigningRequest')}`);
    break;
  }
  case 'export': {
  ...
  case 'upload': {
    ...
    for (const [name, input] of [['APPLE_CERTIFICATE_P12_BASE64', p12], ['APPLE_CERTIFICATE_PASSWORD', passphrase]]) {
      execFileSync('gh', ['secret', 'set', name, '--repo', 'FloSch62/Kubus'], { input, stdio: ['pipe', 'inherit', 'inherit'] });
    }
    console.log('Uploaded both certificate secrets to FloSch62/Kubus.');
    break;
  }
  default:
