...
import { contextName, ghostContextName } from '../helpers/cluster.mjs';
...
test('lists kubeconfig contexts with health over the API', async ({ request }) => {
  ...
  const kind = contexts.find((c) => c.name === contextName);
  expect(kind?.current).toBe(true);
  expect(kind?.health).toBe('connected');

  const ghost = contexts.find((c) => c.name === ghostContextName);
...
test('cluster switcher lists every context including unreachable ones', async ({ page }) => {
  ...
  await expect(page.getByText(ghostContextName)).toBeVisible();
