...
const launcher = process.env.KUBUS_DESKTOP_LAUNCHER ?? path.join(repoRoot, `desktop/build/dev-linux-${process.arch}/Kubus-dev/bin/launcher`);
...
export class NativePage {
  ...
  async press(chord: string): Promise<void> {
    const keys: Record<string, string> = { Backspace: '\uE003', Control: '\uE009', Alt: '\uE00A', Meta: '\uE03D', Tab: '\uE004', Escape: '\uE00C', ArrowDown: '\uE015', ArrowRight: '\uE014', Enter: '\uE007', Space: '\uE00D' };
...
export async function launchDesktop(options: { deepLink?: string; stateDir?: string; kubeconfig?: string } = {}) {
  if (process.platform !== 'linux') throw new Error('Native automation requires Linux WebKitGTK and webkit2gtk-driver.');
  if (!existsSync(launcher)) throw new Error(`Desktop bundle missing: ${launcher}. Run pnpm --filter @kubus/desktop run pack.`);
  const stateDir = options.stateDir ?? mkdtempSync(path.join(tmpdir(), 'kubus-desktop-e2e-'));
  ...
  writeFileSync(kubeconfig, options.kubeconfig ?? JSON.stringify({ apiVersion: 'v1', kind: 'Config', clusters: [], contexts: [], users: [], 'current-context': '' }), { mode: 0o600 });
  const env = { ...process.env, KUBUS_DESKTOP_DATA: userDataDir, KUBECONFIG: kubeconfig, XDG_CONFIG_HOME: path.join(stateDir, 'config'), XDG_CACHE_HOME: path.join(stateDir, 'cache'), KUBUS_NO_OPEN: '1', KUBUS_DEEP_LINK: options.deepLink ?? '' };
  ...
  try {
    ...
    const session = await request<{ sessionId: string }>('/session', { capabilities: { alwaysMatch: { 'webkitgtk:browserOptions': { binary: launcher, args: ['--automation'] } } } });
    ...
    const launched = {
      ...
      openLink: (link: string) => spawn(path.join(path.dirname(launcher), 'kubus-link'), [link], { env, stdio: 'ignore' }),
      async waitForWindow(windowId: string) {
        ...
        await expect.poll(async () => {
          for (const handle of await handles()) {
            ...
            if (await candidate.evaluate((id) => window.kubusDesktop?.windowLaunch?.windowId === id, windowId)) { found = candidate; return true; }
          }
          return false;
        }).toBe(true);
        return found!;
      ...
      async stop() {
        ...
        try {
          for (const handle of await handles()) {
            ...
            await target.evaluate(() => { setTimeout(() => window.kubusDesktop?.closeWindow(), 50); });
